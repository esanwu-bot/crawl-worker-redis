...
class Worker
{
    ...
    private function process(array $item, bool $claimed): void
    {
        ...
        // Agent 任务级控制：cancelled 直接确认跳过；paused 保留在 PEL 等待恢复
        $jobId = (string)($p['job_id'] ?? '');
        if ($jobId !== '') {
            $control = $this->store->getJobControl($jobId);
            if ($control === 'cancelled') {
                $this->store->ack([$msgId]);
                $this->log->info("[{$jobId}] 任务已取消，跳过 {$typeId}/p{$page}");
                return;
            }
            if ($control === 'paused') {
                $this->log->info("[{$jobId}] 任务暂停，保留 {$typeId}/p{$page} 在 PEL");
                return;
            }
        }
        ...
        try {
            ...
            $targetPages = (int)($p['target_pages'] ?? 0);
            if ($targetPages <= 0) {
                $targetPages = $maxPages;
            }
            $reachedEnd = $page >= $lastPage || $page >= $targetPages;
            ...
            if (!$reachedEnd) {
                ...
                if ($this->store->tryLockNextPage($typeId, $page + 1)) {
                    $this->store->addTask($typeId, $page + 1, $typeName, $targetPages, $jobId);
                }
            }
