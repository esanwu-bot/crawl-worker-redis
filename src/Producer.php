...
class Producer
{
     ...
     */
    public function run(array $wantTypes = [], bool $force = false): void
    {
        ...
        foreach ($targets as $t) {
            ...
            if ($force || !$cursor) {
                ...
                $msgId = $this->store->addTask($typeId, 1, $cnName);
                $this->log->info("  [{$typeId}] {$cnName} 已投递首页任务 {$msgId}（max_pages={$maxPages}）");
